...
export async function POST(request: NextRequest) {
  try {
    ...
    drawSection('Ingredients', ingredients.length ? ingredients : [recipeText]);
    drawSection('Instructions', instructions.length ? instructions : [recipeText]);
    drawSection('Nutrition Info', nutritionInfo);

    if (!ingredients.length && !instructions.length && nutritionInfo.length === 0) {
      addText('Recipe Details', { font: helveticaBold, size: 14, color: rgb(0.1, 0.15, 0.25) });
      addText(recipeText, { font: helvetica, size: 11, color: rgb(0.12, 0.12, 0.13) });
    }
